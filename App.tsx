            ...
            case 'cover':
                return (
                    <div className="text-center p-4">
                        <h3 className="text-2xl font-bold text-cyan-400 mb-4">{digitalPackage.ebook.title}</h3>
                        <img src={coverImageUrl} alt="Capa do Ebook" className="mx-auto rounded-lg shadow-2xl shadow-cyan-500/20 max-w-full h-auto md:max-w-md" />
                        <a href={coverImageUrl} download={`capa-${digitalPackage.ebook.title.toLowerCase().replace(/\s+/g, '-')}.png`} className="mt-6 inline-flex items-center gap-2 px-6 py-3 bg-cyan-500 hover:bg-cyan-600 text-white font-bold rounded-lg transition-colors">
            ...
            case 'ebook':
                return (
                    <div className="p-4 space-y-6">
                        <h3 className="text-3xl font-bold text-cyan-400 mb-4">{digitalPackage.ebook.title}</h3>
                        {digitalPackage.ebook.chapters.map((chapter, index) => (
                            <div key={index} className="bg-gray-800/50 p-6 rounded-lg">
                                <h4 className="text-xl font-semibold text-cyan-300 mb-2">{chapter.title}</h4>
            ...
            case 'posts':
                return (
                    <div className="p-4 space-y-4">
                        <h3 className="text-2xl font-bold text-cyan-400 mb-4">Posts para Redes Sociais</h3>
            ...
            case 'bonus':
                return (
                    <div className="p-4 bg-gray-800/50 rounded-lg relative">
                        ...
                        <h3 className="text-2xl font-bold text-cyan-400 mb-2">{digitalPackage.bonus.title}</h3>
            ...
            case 'script':
                return (
                    <div className="p-4 bg-gray-800/50 rounded-lg relative">
                        ...
                        <h3 className="text-2xl font-bold text-cyan-400 mb-4">Script de Vendas</h3>
                    ...
                    <div className="bg-gray-800/50 backdrop-blur-sm p-6 rounded-2xl shadow-lg border border-gray-700">
                        <div className="flex flex-col sm:flex-row gap-4">
                            <input
                                ...
                                className="flex-grow bg-gray-900 border border-gray-600 focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 rounded-lg p-4 text-lg placeholder-gray-500 transition"
                                disabled={isLoading}
                            />
                            <button
                                onClick={handleGeneratePackage}
                                disabled={isLoading || !topic.trim()}
                                className="inline-flex items-center justify-center gap-2 px-8 py-4 bg-gradient-to-r from-cyan-500 to-purple-600 text-white font-bold rounded-lg shadow-md hover:shadow-lg hover:from-cyan-600 hover:to-purple-700 transition-all duration-300 transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed disabled:scale-100"
                            >
                                {isLoading ? (
                                    <>
                                        <Loader />
